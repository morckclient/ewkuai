...
function req_post_auth(url, cookie, callback) {
    let request = net.request({
        url: url,
        method: 'POST',
        headers: {
...
app.whenReady().then(() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            req_get_content(sub_url).then((data) => {
                ...
                if (y2j['proxies'] !== null) {
                    ...
                    y2j['rules'] = ['IP-CIDR,192.168.0.0/16,DIRECT', 'IP-CIDR,10.0.0.0/8,DIRECT', 'IP-CIDR,172.16.0.0/12,DIRECT', 'IP-CIDR,127.0.0.0/8,DIRECT', 'IP-CIDR,100.64.0.0/10,DIRECT', 'GEOIP,CN,Domestic', 'MATCH,Others'];
                    write_yaml(global_config, y2j);
                    resolve('ok');
                } else {
                    resolve('expire');
                }
    ...
    ipcMain.handle('index:getInviteInfo', (event) => {
        return new Promise((resolve, reject) => {
            read_file(auth).then((result) => {
                req_post_auth(domainUrl + '/getuserinviteinfo', result, (result) => {
                    resolve(result)
                })
            });
        })
    ...
    ipcMain.handle('index:sendOrders', (event, args) => {
        return new Promise((resolve, reject) => {
            let postData = `title=${args[0]}&content=${args[1]}&markdown=${args[1]}`;
            read_file(auth).then((result) => {
                req_post_auth(domainUrl + '/user/ticket?' + postData, result, (result) => {
                    resolve(result);
                });
            });
        })
    ...
    ipcMain.handle('index:changePwd', (event, args) => {
        return new Promise((resolve, reject) => {
            let postData = `oldpwd=${args[0]}&pwd=${args[1]}&repwd=${args[2]}`;
            read_file(auth).then((result) => {
                req_post_auth(domainUrl + '/user/password?' + postData, result, (result) => {
                    resolve(result);
                });
            });
        })
    ...
    ipcMain.handle('index:getStorePlan', (event) => {
        return new Promise((resolve, reject) => {
            read_file(auth).then((result) => {
                req_get(domainUrl + '/getusershops', {'cookie': result}, 'application/json', (result) => {
                    resolve(result)
                })
            });
        })
    ...
    ipcMain.handle('index:payWallet', (event, args) => {
        return new Promise((resolve, reject) => {
            let postData = `coupon=&shop=${args}&autorenew=0&disableothers=1`;
            read_file(auth).then((result) => {
                req_post_auth(domainUrl + '/user/buy?' + postData, result, (result) => {
                    resolve(result);
                });
            });
        })
    ...
    ipcMain.handle('index:payPurchase', (event, args) => {
        return new Promise((resolve, reject) => {
